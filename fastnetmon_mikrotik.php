...
$cfg[ ip_mikrotik ] = "185.171.202.254"; // IP Mikrotik Router 
$cfg[ api_user ]    = ""; //user
$cfg[ api_pass ]    = ""; //pass
...
if($ssh = ssh2_connect($cfg[ ip_mikrotik ], 22)) {
    if(ssh2_auth_password($ssh, $cfg[ api_user ], $cfg[ api_pass ])) {
        $stream = ssh2_exec($ssh, '/ip route remove [find dst-address="'.$IP_ATTACK.'/32"]');
